...
- 当用户让你记住任何东西，不要写入公开 `docs/`；应落盘到 ignored 的 `.local-secrets/memorys/`，避免把私有上下文带入 release。
...
## 远程测试规则
- 不要静默运行长时间阻塞命令。如果某个远程步骤可能超过 60 秒，使用可轮询的任务脚本或远程后台 job，并每 30-60 秒向用户报告一次进度。
- 每次远程部署测试或执行会话结束后，除非用户明确要求保持运行，否则及时停止/释放远程机器。
- 永远不要提交本地秘密、凭据、远程机器密码、包含私密数据的诊断报告、`node_modules`、构建缓存或临时传输文件。
