...
- For Azure VM or other remote deployment tests, do not run long blocking commands silently. If a VM step may exceed 60 seconds, start it as a background job with `docs/scripts/azure-vm-start-user-job.ps1`, poll with `docs/scripts/azure-vm-job-status.ps1`, and report progress every 30-60 seconds using status/log tails.
