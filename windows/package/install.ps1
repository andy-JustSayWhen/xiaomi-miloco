...
function Get-PackageTemplateText {
  param([string]$TemplateName)

  $templatePath = Join-Path $PackageTemplateDir $TemplateName
  if (-not (Test-Path -LiteralPath $templatePath)) {
    Fail ("安装包缺少模板文件：{0}" -f $templatePath)
  }
  return [System.IO.File]::ReadAllText($templatePath, [System.Text.UTF8Encoding]::new($false, $true))
}
