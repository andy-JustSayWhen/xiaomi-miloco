# <windows-sample-host> 部署实录

> 关联文档：[部署指南](deployment-guide.md)、[easy-miloco 索引](../index.md)
> 记录对象：远程 Windows 电脑 `<windows-sample-host>`
> 当前状态：已确认应使用 Windows 用户 `<windows-user>` 的 SSH 上下文部署；该用户下 `Ubuntu-24.04` 已存在、正在运行且版本为 WSL2。
> 最近更新：2026-06-22 16:05

## 目标

在远程 Windows 电脑 `<windows-sample-host>` 上部署 Xiaomi Miloco。由于 Miloco 当前不支持原生 Windows，部署路径是：

```mermaid
flowchart TD
  A["Windows 电脑 <windows-sample-host>"] --> B["安装/启用 WSL2"]
  B --> C["进入 Ubuntu-24.04"]
...
- `Ubuntu-24.04` 已经安装在 `<windows-sample-host>` 上。
...
- 公钥免密 SSH 可用。
- 后续部署可以由本机 Codex 通过 SSH 远程接管 `<windows-sample-host>`。
- 目标用户 `sshadmin` 是 Windows 本地管理员账户，适合执行 WSL 和系统配置检查。

### 阶段 5：确认部署必须使用 <windows-user> 用户上下文
...
```text
whoami -> win<wsl-user>\<windows-user>
...
- 后续 Miloco 部署必须在 `<windows-user>` 用户上下文中执行，避免把服务和配置装到 `sshadmin` 下面。
...
- `<windows-sample-host>` 使用 Microsoft 账户登录。
- Microsoft 账户邮箱：`<MICROSOFT_ACCOUNT_EMAIL>`
...
- 只用 PIN/Windows Hello 登录通常不能刷新 SSH 可用的密码凭据；更可靠的方式是联网状态下注销 `<windows-user>` 会话，再用“密码”方式登录一次。
...
用户补充：已经通过网易 UU 打开 `<windows-sample-host>`，并打开了远程终端；要求验证 GUI 兜底能力。

验证结果：

- 当前 Codex 会话没有暴露可直接点击/截图的 `computer-use` 工具；插件安装候选列表中也没有该插件。
- 通过 SSH 在 `<windows-user>` 的交互会话里创建计划任务抓取桌面截图成功，确认 console 桌面画面可见。
...
用户睡前要求先确认 `[@电脑](plugin://computer-use@openai-bundled)` 是否可用，并验证网易 UU 远程里是否能看到 `<windows-sample-host>`。
...
```javascript
globalThis.targetWindow = await sky.get_window(targetApp.windows.find(w => w.title === "<windows-sample-host>"));
...
```text
window.title = <windows-sample-host>
...
- 截图中可见 `<windows-sample-host>` 的 Windows 桌面，说明副屏/远程画面通路可作为兜底观察方式。
...
- <windows-sample-host> 当前仍处于“基础服务与 OpenClaw 插件就绪”。
...
- [<windows-sample-host>后授权收尾Runbook](windows-sample-host-post-auth-runbook.md)
...
- 收到后按 [<windows-sample-host>后授权收尾Runbook](windows-sample-host-post-auth-runbook.md) 继续执行。
...
- <windows-sample-host> 运行状态稳定。
- 满血部署继续等待小米 OAuth 授权码和 MiMo API Key。

### 阶段 22：新增预检/验收脚本并在 <windows-sample-host> 实跑
...
- 脚本化验收与人工复核一致：<windows-sample-host> 基础部署可用，OpenClaw 插件链路可用。
...
- 收到后不再手动分散执行多条命令，优先使用该脚本收尾；失败时再按 [<windows-sample-host>后授权收尾Runbook](windows-sample-host-post-auth-runbook.md) 的手动步骤定位。
...
- 统一入口已在 <windows-sample-host> 验证可用。
...
- 把 <windows-sample-host> 本次实际遇到的状态分支抽象成“我现在处在哪一步，下一条命令是什么”的决策树。
...
- 这一步不改变 <windows-sample-host> 当前运行状态。
- 当前远端状态仍为基础服务就绪、OpenClaw 插件就绪、等待小米 OAuth payload 和 MiMo API Key。

### 阶段 26：新增一键诊断报告并留档 <windows-sample-host> 快照
...
```powershell
ssh <windows-user>@<tailscale-ip> "powershell.exe -NoProfile -ExecutionPolicy Bypass -File C:\Users\<user>\AppData\Local\Temp\win-miloco-workflow.ps1 -Action Report -Distro Ubuntu-24.04 -MilocoPort 1886 -OpenClawPort 18789 -ReportPath C:\Users\<user>\AppData\Local\Temp\miloco-windows-sample-host-report.txt"
```

远端报告：

```text
C:\Users\<user>\AppData\Local\Temp\miloco-windows-sample-host-report.txt
```

已拉回 OB 留档：

```text
02-deploy/reports/windows-sample-host-20260622-035220-report.txt
```
...
- 诊断报告生成能力已在 <windows-sample-host> 验证。
...
- <windows-sample-host> 当前只能判定为基础服务和 OpenClaw 插件就绪。
- 不能判定满血部署完成。
...
- 把 <windows-sample-host> 的差异标记为实机适配：`1810` 改 `1886`、用户目录安装 Node/OpenClaw、显式代理、外层预检/验收脚本。

判断：

- 当前基础部署仍与官方流程一致。
- `BASIC_READY=yes` 不等于满血；官方快速开始要求配置模型、绑定账号、开启摄像头感知，<windows-sample-host> 仍等待小米 OAuth payload 和 MiMo API Key。
...
- [<windows-sample-host>授权阶段用户操作卡片](windows-sample-host-auth-card.md)
...
- <windows-sample-host> 仍处于基础就绪、等待小米 OAuth payload 和 MiMo API Key 的状态。
...
```powershell
ssh <windows-user>@<tailscale-ip> "powershell.exe -NoProfile -ExecutionPolicy Bypass -File C:\Users\<user>\AppData\Local\Temp\win-miloco-workflow.ps1 -Action Report -Distro Ubuntu-24.04 -MilocoPort 1886 -OpenClawPort 18789 -ReportPath C:\Users\<user>\AppData\Local\Temp\miloco-windows-sample-host-report-20260622-0412.txt"
```
...
- <windows-sample-host> 当前不满血不是教程缺口，而是等待用户提供小米 OAuth payload 和 MiMo API Key。
...
- 资料包脚本当前语法通过。
- <windows-sample-host> 满血状态仍等待小米 OAuth payload 和 MiMo API Key。
...
- 这一步不改变 <windows-sample-host> 运行状态。
- 当前仍等待小米 OAuth payload 和 MiMo API Key。
...
- 这一步只生成分发资料，不改变 <windows-sample-host> 服务状态。
...
- 这只证明分发资料完整，不代表 <windows-sample-host> 已满血；当前仍等待小米 OAuth payload 和 MiMo API Key。
...
- 下一步只需要用户完成 OAuth 授权并提供 MiMo API Key，然后执行 [<windows-sample-host>后授权收尾Runbook](windows-sample-host-post-auth-runbook.md)。
...
- 这一步仍不改变 <windows-sample-host> 运行状态；满血仍等待小米 OAuth payload 和 MiMo API Key。
...
- 这一步不改变 <windows-sample-host> 服务状态；仍等待小米 OAuth payload 和 MiMo API Key。
...
### 阶段 46：新增 <windows-sample-host> 部署完成度审计

新增文档：

- [<windows-sample-host>部署完成度审计](windows-sample-host-readiness-audit.md)
...
- 阶段 46 新增 [<windows-sample-host>部署完成度审计](windows-sample-host-readiness-audit.md)。
...
- 这一步不改变 <windows-sample-host> 服务状态。
...
```powershell
ssh <windows-user>@<tailscale-ip> "powershell.exe -NoProfile -ExecutionPolicy Bypass -File C:\Users\<user>\AppData\Local\Temp\win-miloco-workflow.ps1 -Action Report -Distro Ubuntu-24.04 -MilocoPort 1886 -OpenClawPort 18789 -ReportPath C:\Users\<user>\AppData\Local\Temp\miloco-windows-sample-host-report-20260622-044254.txt"
scp <windows-user>@<tailscale-ip>:C:/Users/<user>/AppData/Local/Temp/miloco-windows-sample-host-report-20260622-044254.txt E:\BaiduSyncdisk\obsidian repo\default\App学习笔记\easy-miloco\02-deploy\reports\windows-sample-host-20260622-044254-report.txt
```
...
### 阶段 49：核对 <windows-sample-host> 临时脚本与 OB 当前脚本一致
...
<windows-sample-host> 临时目录脚本 SHA256：
...
- <windows-sample-host> 临时脚本与 OB 当前脚本完全一致，不需要重传。
- 后授权收尾时可以直接调用 <windows-sample-host> 临时目录里的 `win-miloco-workflow.ps1 -Action Finish`。
- 当前仍是授权前状态，不是基础链路失败。
...
- 阶段 49 补充了 <windows-sample-host> 临时脚本与 OB 当前脚本一致性证据。
- 可分发资料包需要包含最新的 [<windows-sample-host>后授权收尾Runbook](windows-sample-host-post-auth-runbook.md)。
...
- 重新生成包内目录，纳入 [Windows部署资料包版本说明](release-notes-template.md) 和 [<windows-sample-host>后授权收尾Runbook](windows-sample-host-post-auth-runbook.md)。
...
## 2026-06-22 04:51 <windows-sample-host> 授权前重验
...
```text
02-deploy/reports/windows-sample-host-20260622-045716-report.txt
```
...
- 目标机基础服务继续稳定。
- 当前唯一推进动作仍是完成小米 OAuth 授权，并提供 MiMo API Key；收到后执行 [<windows-sample-host>后授权收尾Runbook](windows-sample-host-post-auth-runbook.md)。
...
- [<windows-sample-host>部署完成度审计](windows-sample-host-readiness-audit.md) 的当前报告引用已从 `044254` 更新为 `045716`。
...
BASH_PARSE_PASS wsl-miloco-validate.sh
BASH_PARSE_PASS wsl-post-auth-finish.sh
CompletionAuditReport=reports/windows-sample-host-20260622-045716-report.txt
```
...
```text
02-deploy/reports/windows-sample-host-20260622-050326-report.txt
```
...
- 改用 PowerShell `-EncodedCommand` 后 `LaunchExit=0`，已尝试在 <windows-sample-host> 默认浏览器拉起授权页。

脚本修复：

- `win-miloco-workflow.ps1` 新增 `-OmniModel` 和 `-OmniBaseUrl`。
- 后续 <windows-sample-host> 收尾必须显式传：
...
- 已在 <windows-sample-host> 桌面放置兜底文件：
...
```text
02-deploy/reports/windows-sample-host-20260622-101157-oauth-ready.txt
```
...
- Miloco 后端运行在 `http://127.0.0.1:1886/`，原因是 <windows-sample-host> 的 Windows TCP excluded port range 占用了默认 `1810`。
...
- 后授权一键收尾脚本已上传到 <windows-sample-host> 临时目录，并已生成最新小米账号绑定链接。
- Windows 统一入口 `win-miloco-workflow.ps1` 已上传到 <windows-sample-host> 临时目录，并通过 `AllBasic`、`BindUrl` 验证。
- 已新增 [Windows部署决策树](decision-tree.md)，用于其他 Windows 玩家按状态选择下一步。
- 已生成并留档诊断报告：`02-deploy/reports/windows-sample-host-20260622-035220-report.txt`。
- 已新增 [Windows部署总入口](index.md)，作为 Windows 玩家部署第一入口。
- 已新增 [Windows满血验收证据清单](full-validation-evidence.md)，作为最终交付证据标准。
- 已新增 [官方部署流程对齐核查](upstream-deploy-alignment.md)，确认教程和 <windows-sample-host> 实机适配没有偏离官方 installer 主线。
- 2026-06-22 04:04 再次复核：`BASIC_READY_FROM_WINDOWS=yes`、`BASIC_READY=yes`、`FULL_READY=no`，并重新生成 OAuth 链接。
- 已新增 [<windows-sample-host>授权阶段用户操作卡片](windows-sample-host-auth-card.md)，作为用户回来后的最短操作入口。
- 已补强“任意 Windows 情况”覆盖：旧 WSL/DISM 兜底、Agent 输入清单、官方 `--agent-finish` 参数、远程 scp 路径；2026-06-22 04:09 复核状态仍为 `FULL_READY=no`。
- 已生成并留档 04:10 最新诊断报告：`02-deploy/reports/windows-sample-host-20260622-041058-report.txt`。
...
- 已新增 [<windows-sample-host>部署完成度审计](windows-sample-host-readiness-audit.md)，逐项记录当前已完成证据和未完成的满血缺口。
- 2026-06-22 重建 Windows 部署资料包以纳入完成度审计，最终 zip SHA256 为 `E49611FDB6CAB3071B221D86DD7AA1F71B85540106062165E83A29083EDAE173`。
- 2026-06-22 04:43 生成最新诊断报告 `reports/windows-sample-host-20260622-044254-report.txt`：基础链路继续 0 失败，`FULL_READY=no`，账号和 Key 仍缺。
- 2026-06-22 04:46 核对 <windows-sample-host> 临时脚本与 OB 当前脚本 SHA256 完全一致，远端 `-Action Validate` 可执行，返回 `BASIC_READY=yes`、`FULL_READY=no`、`FAIL_COUNT=0`。
- 2026-06-22 重建资料包以纳入最新 Runbook 和版本说明，最终 zip SHA256 为 `D38EC45C032FE7293FBA4F5D685AC4F9E23D928EF90E8890CEE6486F93271963`。
- 2026-06-22 04:57 生成最新诊断报告 `reports/windows-sample-host-20260622-045716-report.txt`，并刷新小米 OAuth 入口；远端仍为 `BASIC_READY=yes`、`FULL_READY=no`、`FAIL_COUNT=0`。
- 2026-06-22 04:58 复核官方部署入口后重建资料包，最终 zip SHA256 为 `8D8B6137B17718CC9F5ED160A30E0E38A59FBBBCEF0E5FA98997FD9E84DBC63E`。
- 2026-06-22 05:03 授权前复核无变化，留档 `reports/windows-sample-host-20260622-050326-report.txt`；远端仍为 `BASIC_READY=yes`、`FULL_READY=no`、`FAIL_COUNT=0`。
...
- 2026-06-22 10:11 刷新并拉起小米 OAuth 链接，在 <windows-sample-host> 桌面放置 `miloco-xiaomi-oauth.url` 和 `miloco-xiaomi-oauth.txt`，留档 `reports/windows-sample-host-20260622-101157-oauth-ready.txt`。
...
同步留档：
```text
02-deploy/reports/windows-sample-host-20260622-101542-oauth-relaunched.txt
```

判断：
- 授权页已再次拉起。
- 桌面兜底入口已刷新。
- 当前仍等待用户在 <windows-sample-host> 浏览器完成小米登录授权，并把跳转后的回调 URL、`code` 或完整 payload 发回。
...
处理：
```text
先 activate_window(<windows-sample-host>)，再双击桌面左侧的 Miloco 小米授权快捷方式。
```
...
同步留档：
```text
02-deploy/reports/windows-sample-host-20260622-101729-oauth-page-visible.txt
```

判断：
- 电脑插件可用，能够看到并控制网易 UU 远程中的 `<windows-sample-host>` 窗口。
...
[camera] Connected device: <camera-did-desk> (<camera-desk>)
HTTP Request: POST https://token-plan-sgp.xiaomimimo.com/v1/chat/completions "HTTP/1.1 200 OK"
realtime_perceive 输出<room-bedroom>画面描述
```
...
- <windows-sample-host> 已按官方流程完成 Miloco + OpenClaw + 小米账号 + MiMo 视觉模型 + 设备 + 摄像头感知闭环。
...
```text
Computer Use plugin -> 网易 UU 远程 -> <windows-sample-host>
```

过程：

- 远程 Chrome 仍能看到小米 `授权成功` 页面，确认用户侧 OAuth 浏览器流程成功。
- 在 <windows-sample-host> 桌面创建并双击 `Miloco 面板.url`。
- 打开 `http://127.0.0.1:1886/`，Miloco 面板正常加载。
- 概览页显示账号 `mdidb`、`127 个设备`，实时画面卡片带 `LIVE`，能看到<room-bedroom>摄像头画面。
...
- 在 <windows-sample-host> 桌面创建并双击 `OpenClaw Gateway.url`。
...
```text
02-deploy/reports/windows-sample-host-20260622-103621-ui-smoke.txt
```
...
- Miloco 概览页仍能看到<room-bedroom>实时画面卡片，设备页仍显示 `127 devices`、`13 rooms`，属于只读 / 安全替代验证。
...
```text
<windows-sample-host> / Tailscale: <tailscale-ip>
...
```text
<camera-did-desk> | <camera-desk> | <room-bedroom> | chuangmi.camera.061a01 | is_set_pincode=0
<camera-did-bedside>  | <camera-bedside>   | <room-bedroom> | chuangmi.camera.036a02 | is_set_pincode=1
<camera-did-living> | <camera-living>   | <room-living> | chuangmi.camera.021a04 | is_set_pincode=0
```
...
结果：
- `<camera-did-living> / <camera-living>` 开始能 `record_clip` 和 perception。
- `<camera-did-bedside> / <camera-bedside>` 仍不能产出 keyframe。
...
```text
<camera-did-bedside>  | <camera-bedside>   | <room-bedroom> | is_online=true | in_use=true | connected=true
<camera-did-desk> | <camera-desk> | <room-bedroom> | is_online=true | in_use=true | connected=true
<camera-did-living> | <camera-living>   | <room-living> | is_online=true | in_use=true | connected=true
```
...
```text
<camera-did-desk> => <room-bedroom>书桌、键盘、鼠标、带吸管杯子、黑色置物架、紫色瑜伽球，房间无人。
<camera-did-bedside>  => <room-bedroom>内电脑显示器亮着，桌面有键盘鼠标，房间安静无人。
<camera-did-living> => <room-living>内无人或可见<room-living>沙发/人物，能返回实时画面描述。
```

实时感知日志中出现三路合并：

```text
[multicam] n_cam=3 | <camera-did-desk>-<camera-desk> | <camera-did-living>-<camera-living> | <camera-did-bedside>-<camera-bedside>
_device_count=3
```

### OpenClaw 聊天页最终验收

由于 UU 远程窗口显示黑屏，改用 SSH 隧道把 <windows-sample-host> 的 loopback gateway 暴露到本机：
...
<camera-did-desk> <camera-desk>   <room-bedroom>  ✅      ✅      ✅
<camera-did-bedside>  <camera-bedside>     <room-bedroom>  ✅      ✅      ✅
<camera-did-living> <camera-living>     <room-living>  ✅      ✅      ✅

2. 实时视觉感知结果
① <camera-desk>（<camera-did-desk>）
前景白色桌面上有黑色键盘、鼠标、带吸管白色杯子；背景黑色置物架，地上紫色瑜伽球；无人无宠物。

② <camera-bedside>（<camera-did-bedside>）
画面中央有亮着的电脑显示器，桌面有键盘和鼠标，右侧有衣架和衣物；没有看到人或宠物。

③ <camera-living>（<camera-did-living>）
<room-living>里一名女孩坐在沙发上，一名男孩站在右侧，女孩正伸手指着男孩。
...
```text
<camera-did-bedside> / <camera-bedside>：is_online=false, connected=false
...
同一时刻从 <windows-sample-host> WSL 直接重试 `<camera-did-bedside>` 又能成功返回视觉描述，说明设备、PIN 和画面链路本身未坏，问题集中在 Miloco on-demand 感知的冷启动/短暂掉线窗口：`PerceptionService.on_demand_perceive` 只读取 collector 当前 active sources；如果 450 刚好不在 active 集合中，就立即 2011，不会主动等待设备 sync 或新帧进入缓冲区。

### 运行时修复
在 <windows-sample-host> 的已安装 Miloco 中给 on-demand 增加短等待与主动 sync：
...
=> code=0
=> 画面：<room-bedroom>场景；电脑显示器亮着；桌面有键盘和鼠标；右侧衣架/衣物；旁边纸箱；前景黑色线缆；无人或宠物。
```

### OpenClaw 聊天页最终复验
通过本机到 <windows-sample-host> 的 gateway 隧道连接真实 OpenClaw 主聊天：
...
```text
<camera-did-desk> | <camera-desk> | 能 | 桌面有键盘、鼠标、带吸管保温杯；背景置物架、紫色瑜伽球等；无人无宠物。
<camera-did-bedside>  | <camera-bedside>    | 能（重试成功） | 线缆、亮着的电脑显示器、置物架杂物、衣柜衣物、纸箱等。
<camera-did-living> | <camera-living>   | 能 | L 型沙发、茶几、纸巾盒、小篮子、洗衣机、收纳箱；有人从右侧走向门口。
```
...
用户在 15:49 提供 OpenClaw 聊天页截图，截图中 `<camera-did-bedside> / <camera-bedside>` 再次显示：
...
在 <windows-sample-host> / WSL 中重新检查：
...
画面：安静<room-bedroom>；电脑显示器开启并发出白光；左侧金属架；右侧纸箱和衣物；无人，环境静止。
```
...
```text
<camera-did-desk> | <camera-desk> | 能 | 白色书桌、键盘、鼠标、白色圆柱形设备、小物件、黑色置物架、紫色瑜伽球；无人无宠物。
<camera-did-living> | <camera-living>   | 能 | L 型沙发、茶几、纸巾盒、编织篮、白色柜子、绿色塑料收纳箱；无人无宠物。
<camera-did-bedside>  | <camera-bedside>    | 能（第三次重试成功） | 昏暗房间，电脑显示器亮着，显示器前有键盘鼠标，左侧金属衣架挂衣物，右侧白色柜子或箱子及杂物；无人。
```
...
- 后续如果继续提高稳定性，应把验收口径升级为“<room-bedroom>双摄连续 N 轮、无需重试均通过”，不能只凭一次最终成功宣布稳定。

原始留档：

```text
[reports/windows-sample-host-20260622-155600-openclaw-camera-regression-after-screenshot.txt](reports/windows-sample-host-20260622-155600-openclaw-camera-regression-after-screenshot.txt)
```
...
- OpenClaw 中 `<camera-did-bedside> / <camera-bedside>` 已能返回视觉画面，但聊天页左上角曾出现 `Nodes: '<camera-bedside>' failed`。
- Miloco WebUI 的“此刻”页实时画面已能看到其他摄像头，但下方 `miloco 未感知设备` 区域里 `<camera-bedside>` 曾显示 `已离线 · <room-bedroom>`。
...
```text
<camera-did-desk> | <camera-desk> | 能 | 白色桌面、黑色键盘、鼠标、白色水杯、小物件、黑色置物架、紫色瑜伽球；无人无宠物。
<camera-did-bedside>  | <camera-bedside>   | 能 | <room-bedroom>场景；电脑显示器亮着，屏幕下方有键盘鼠标；左侧金属架子；右侧衣物和箱子；无人。
<camera-did-living> | <camera-living>   | 能（重试成功） | <room-living>、灰色瓷砖、棕色沙发、茶几、洗衣机、蓝色收纳箱；远处有人背对镜头整理物品。
```
...
注意：这轮 OpenClaw 复验中，`<camera-did-living> / <room-living>` 在 backend 刚重启后出现一次 transient `cannot connect to Miloco backend at http://127.0.0.1:1886`，随后重试成功；`<camera-did-bedside> / <camera-bedside>` 本轮没有再作为离线设备失败。

原始留档：

```text
[reports/windows-sample-host-20260622-170800-connected-camera-online-state-fix.txt](reports/windows-sample-host-20260622-170800-connected-camera-online-state-fix.txt)
```
...
但本轮对当前 <windows-sample-host> 使用的 OpenAI-compatible endpoint 做了直接 API 测试：
...
结论：<windows-sample-host> 这套 OpenClaw 当前走的是 OpenAI-compatible `chat/completions` 路径，不能直接把模型 ID 改成 `[1m]` 后缀，否则会不可用。正确处理是保持模型 ID 为 `mimo-v2.5-pro`，只把 OpenClaw 本地模型元数据里的上下文窗口改为 1M。
...
OpenClaw 仍然是运行时和插件平台，但 <windows-sample-host> 当前用户入口默认会被 Miloco 牵引：
...
当前 <windows-sample-host> 部署中，Miloco 没有把 OpenClaw 阉割成专用壳；它是通过插件机制把普通 OpenClaw 主对话改造成 Miloco 家庭智能体体验。
...
| 层级 | 准确叫法 | 证据 | 用户能看到什么 |
|---|---|---|---|
| 安装/运行时层 | OpenClaw plugin | `plugins/openclaw/openclaw.plugin.json` 声明 `id=miloco-openclaw-plugin`、`activation.onStartup`、`hooks`、`contracts.tools`、`skills`；<windows-sample-host> 实机 `~/.openclaw/openclaw.json` 里 `plugins.entries.miloco-openclaw-plugin.enabled=true`。 | 通常不一定在当前 Web 首页显式露出“插件”菜单。 |
...
把 <windows-sample-host> 桌面入口改为真正的启动器：
...
这会和桌面启动器抢同一个后端进程，表现为后端已经 `Server is ready`，随后马上收到 SIGTERM 退出。为避免两套机制互相打架，已在 <windows-sample-host> 的已安装插件里把 `miloco-backend` service 改成 no-op，只记录日志，不再启停后端：
...
现在 <windows-sample-host> 桌面上的 `OpenClaw Gateway` 和 `Miloco 面板` 已不是普通网页链接，而是“检查服务 -> 必要时拉起 WSL 后台进程 -> 打开网页”的启动器；前端配置只有在后端 API 可用并保存成功后才会生效。

## 2026-06-22 18:47-19:00 OpenClaw 接入飞书消息渠道

### 目标

把 <windows-sample-host> 上的 OpenClaw 接入飞书，使 OpenClaw agent 能通过飞书发消息，并让 Miloco 的 `miloco_im_push` 可以把家庭通知推送到飞书。
...
| 9 | `openclaw message send --channel feishu ...` | 普通 Feishu 出站消息发送成功，消息 ID：`<FEISHU_MESSAGE_ID>`。 |
...
```json
{
  ...
  "botOpenId": "<FEISHU_OPEN_ID>",
  "authorizedUserOpenId": "<FEISHU_OPEN_ID>"
}
```
...
```json
{
  "notifySessionKey": "agent:main:feishu:dm:<FEISHU_OPEN_ID>",
  "sessionEntry": {
    "lastChannel": "feishu",
    "lastTo": "<FEISHU_OPEN_ID>",
...
| Feishu 探针成功 | `probe.ok=true`，`botName=openclaw with miloco`，`botOpenId=<FEISHU_OPEN_ID>`。 |
| 普通消息出站成功 | `openclaw message send` 返回 `ok=true`，消息 ID：`<FEISHU_MESSAGE_ID>`。 |
| Agent 通过飞书回复成功 | `deliverySucceeded=true`，回复文本为“OpenClaw 已接入飞书，我可以通过飞书回复你了。” |
| Miloco 通知绑定成功 | `notifySessionKey=agent:main:feishu:dm:<FEISHU_OPEN_ID>`，session entry 有 `lastChannel=feishu`、`lastTo=<open_id>`。 |
...
截至 2026-06-22 19:00，<windows-sample-host> 上 OpenClaw 已接入飞书，且 Miloco 的 IM 通知也已绑定到飞书并通过 Gateway 路径验收。当前已验证“OpenClaw/agent 出站投递”和“Miloco 主动通知推送”；还未验证“用户从飞书客户端发消息给 bot 后 OpenClaw 自动回复”的真实入站链路。
...
<windows-sample-host> 上又出现摄像头离线：`<camera-did-desk> / <camera-desk>` 在 Miloco 摄像头列表里被判为 `is_online=false`，但同一台设备在米家云端 `/api/miot/home` 仍是 `online=true`，并且 WSL LAN 网卡 `eth2` 能 ping 通摄像头 IP `<LAN_IP>`，ARP 可见 `<MAC_ADDRESS>`。
...
| <windows-sample-host> 热修 | `/home/<wsl-user>/.local/share/uv/tools/miloco/lib/python3.12/site-packages/miloco/perception/collect/camera_adapter.py` | 已把补丁同步到实际运行的 site-packages，并通过 `py_compile` |
...
|---|---|---:|---:|---:|
| `<camera-did-desk>` | <camera-desk> | true | true | true |
| `<camera-did-living>` | <camera-living> | true | true | true |
| `<camera-did-bedside>` | <camera-bedside> | true | true | true |
...
第一次修复后，`<camera-did-desk> / <camera-desk>` 又出现新问题：Miloco WebUI 卡片和弹窗显示 `LIVE`/正在连接，但画面黑屏，随后提示 `连不上摄像头（可能不在同一局域网，或摄像头离线）`。这说明“是否订阅成功”和“是否真的拿到画面”之间仍然被混为一谈。
...
已热更新 <windows-sample-host> 实际运行包：
...
|---|---|---:|---:|---:|
| `<camera-did-bedside>` | <camera-bedside> | true | true | true |
| `<camera-did-desk>` | <camera-desk> | false | true | false |
| `<camera-did-living>` | <camera-living> | true | true | true |

WebUI 验收：

- 首页实时画面显示 `2 个在感知`。
- 只展示 `<room-living>` 与 `<room-bedroom>/<camera-bedside>` 两个 LIVE 卡片。
- `<camera-desk>` 进入 `miloco 未感知设备`，状态为 `已离线 · <room-bedroom>`。
...
| <windows-sample-host> 热修复 | 已同步到 site-packages 并重启 Miloco 服务，当前 `pid=940964` |
...
<camera-did-living> <camera-living> online=True in_use=True connected=True
<camera-did-bedside> <camera-bedside> online=True in_use=True connected=True
<camera-did-desk> <camera-desk> online=False in_use=True connected=False
ENGINE
[('<camera-did-bedside>', '<camera-bedside>'), ('<camera-did-living>', '<camera-living>')]
```
...
| did | 摄像头 | 当前结论 | 证据 |
|---|---|---|---|
| `<camera-did-living>` | <camera-living> | 正常 | `connected=true`，`active_sources` 包含该 did，实时感知持续描述<room-living>画面 |
| `<camera-did-bedside>` | <camera-bedside> | 已修复 | SDK 日志曾报 `pin code is required`；传入 PIN 后进入 `active_sources`，实时感知能描述<room-bedroom>画面 |
| `<camera-did-desk>` | <camera-desk> | 仍未能出帧，根因偏物理网络/PPCS | `online=true` 但 `lan_online=false`、`local_ip=null`；直连 SDK 无 PIN 断开，带 `<CAMERA_PIN>` 后 `result->0` 但仍无 raw/decoded frame；日志有 `PPCS_Connect errorcode: -3` |

已排除项：

- WSL 网络模式：`C:\Users\<user>\.wslconfig` 为 `networkingMode=Mirrored`。
- WSL LAN 网卡：`eth2=<LAN_IP>/24`，与可用摄像头 `<LAN_IP>`、`<LAN_IP>` 同网段。
...
```text
<camera-did-living> <camera-living>       online=true  lan_online=true   local_ip=<LAN_IP>   connected=true
<camera-did-bedside>  <camera-bedside>       online=true  lan_online=false  local_ip=<LAN_IP>  connected=true
<camera-did-desk> <camera-desk>     online=true  lan_online=false  local_ip=null            connected=false
```
...
本轮先完成 Git 与远端热修复同步，再重新核验 <windows-sample-host> 上三台摄像头。结论必须分开写：
...
<camera-did-bedside>  <camera-bedside>      online=true in_use=true connected=true
<camera-did-living> <camera-living>      online=true in_use=true connected=true
<camera-did-desk> <camera-desk>    online=true in_use=true connected=false
```
...
| did | 摄像头 | 当前状态 | 证据 | 结论 |
|---|---|---|---|---|
| `<camera-did-living>` | <camera-living> | `online=true in_use=true connected=true` | OpenClaw/Miloco 感知日志出现<room-living>画面描述 | 正常 |
| `<camera-did-bedside>` | <camera-bedside> | `online=true in_use=true connected=true` | PIN `<CAMERA_PIN>` 生效；stall 后 manager 可重建；感知日志出现<room-bedroom>画面描述 | 已修复，需继续观察稳定性 |
| `<camera-did-desk>` | <camera-desk> | `online=true in_use=true connected=false` | 单播 LAN 可发现，PIN `<CAMERA_PIN>` 被接受，但 SDK 长时间 `raw=0 decoded=0`，日志持续 `PPCS_Connect errorcode: -3` | 未修复，根因在该机型/设备/PPCS 视频数据面 |
...
| 不是 Miloco 看不到设备 | LAN 单播 `ping_async(target_ip=<LAN_IP>)` 可发现该 did，并能把 `is_online` 修正为 true |
| 不是全局 WSL 网络失败 | 同一 <windows-sample-host>/WSL 环境下 `<camera-did-living>`、`<camera-did-bedside>` 都能出帧 |
...
1. 在米家 App 确认 `<camera-desk>` 能实时预览。
