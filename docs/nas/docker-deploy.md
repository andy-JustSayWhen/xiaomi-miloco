...
部分 NAS 的 Docker Hub 镜像源会缓存同名 tag。若 `docker compose pull` 显示成功但启动日志仍出现旧的 `WSL Miloco validation` 或模型缺失，改用下面这个 digest 固定镜像：

```text
docker.io/andywu114/easy-miloco-nas:v0.5@sha256:9790f00cc511ab9a4132c8650150b27807fa5292e17c47354f9cfc484ab58370
```

面向普通用户的一键 YAML 建议先用 digest 固定版本；维护者日常开发才使用裸 `v0.5` tag。
...
# v0.5 首包推荐：绕过 NAS 镜像源 tag 缓存
EASY_MILOCO_IMAGE=docker.io/andywu114/easy-miloco-nas:v0.5@sha256:9790f00cc511ab9a4132c8650150b27807fa5292e17c47354f9cfc484ab58370 ./manage.sh start
