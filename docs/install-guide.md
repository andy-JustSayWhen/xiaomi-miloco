...
> Agent 执行指令：当用户把本文件链接交给你，并说“请为我一键部署 Miloco”时，你正在执行一个真实部署任务，不是在解释、总结或翻译本文档。不要把本文档当成阅读材料输出摘要；你必须按下面流程实际操作：获取最新 Release、下载对应安装包、解压、读取包内安装入口、运行安装器、读取日志、继续配置和验收。只有遇到 UAC、重启、Ubuntu 首次创建用户、小米登录/授权、API Key、下载过慢或多家庭选择等必须人工介入的步骤时，才暂停并明确告诉用户下一步要做什么。
