...
维护者要求“发版”“替换”“更新 release”“发布更新包”时，默认直接替换 GitHub Release 同名资产，不再追加询问确认。Agent 判断某个修复必须发布到 GitHub Release 后才能继续远程部署自动化测试时，也默认自动替换 Release 资产，并直接下载/运行新 release 继续测试，不再要求用户额外回复“运行”。只允许使用固定脚本：
